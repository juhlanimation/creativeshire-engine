# Sections

**Page-level containers for widgets.**

Before creating:
- Is this a page-level container for widgets?
- Does the pattern exist already? Check patterns/ first.
- Is it generalized? Settings should allow many visual variations.

Specs:
- [section.spec.md](/.claude/skills/creativeshire/specs/components/content/section.spec.md)
- [section-pattern.spec.md](/.claude/skills/creativeshire/specs/components/content/section-pattern.spec.md)
