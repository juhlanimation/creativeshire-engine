...
const checkExtendsPreset: ValidationCheck = (content, filePath) => {
  ...
  if (!content.includes('creativeshire/presets/')) {
    return 'config.ts must import and extend a preset from creativeshire/presets/'
  }
 ...
 * From spec: "No imports from creativeshire/experience/ directly"
 */
const checkNoExperienceImports: ValidationCheck = (content, filePath) => {
  ...
  if (/from\s+['"].*creativeshire\/experience\//.test(content)) {
    return 'Page files cannot import from creativeshire/experience/ - use schema to declare behaviour'
  }
...
/**
 * Check: Data files are pure (no creativeshire imports)
 * From spec: "No imports from creativeshire in data files"
 */
const checkDataFilesPure: ValidationCheck = (content, filePath) => {
  // Only check data files
  if (!filePath.includes('/data/')) return null

  if (/from\s+['"].*creativeshire\//.test(content)) {
    return 'Data files must not import from creativeshire - pure data only'
  }
...
/**
 * Check: No creativeshire code modification
 * From spec: "No modification of creativeshire code"
...
const checkNoCreativeshireMutation: ValidationCheck = (content) => {
  // Check for assignment to creativeshire imports
