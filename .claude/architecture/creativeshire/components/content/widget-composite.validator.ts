#!/usr/bin/env npx tsx
/**
 * Widget Composite Validator
 *
 * Validates files in creativeshire/components/content/widgets/composite/
 *
 * Spec: ./widget-composite.spec.md
 *
 * Exit codes:
...
const VALIDATOR_NAME = 'widget-composite'

// Path pattern for this domain
// Use forward slashes - paths are normalized before matching
const DOMAIN_PATH = /creativeshire\/(?:components\/)?content\/widgets\/composite\//
 ...
 * From spec: "Function named create{Name}"
 */
const checkFactoryNaming: ValidationCheck = (content, filePath) => {
  if (!filePath.endsWith('index.ts')) return null

  // Check for export function create{Name}
  if (!/export\s+function\s+create[A-Z]/.test(content)) {
    return 'Factory function must be named create{Name} (e.g., createProjectCard)'
  }
  return null
}

/**
 * Check: Return type is WidgetSchema
 * From spec: "Return type is WidgetSchema"
 */
const checkReturnType: ValidationCheck = (content, filePath) => {
  if (!filePath.endsWith('index.ts')) return null

  // Check for WidgetSchema return type annotation
  if (!/:\s*WidgetSchema\s*[{=]/.test(content) && !content.includes('): WidgetSchema')) {
    return 'Factory function must have explicit WidgetSchema return type'
  }
  return null
}
...
/**
 * Check: No CSS files
 * From spec: "No CSS files - composites produce schema, not styled components"
 */
const checkNoCssFiles: ValidationCheck = (content, filePath) => {
  if (filePath.endsWith('.css')) {
    return 'CSS files not allowed in composite directory - composites produce schema only'
  }
  return null
}
...
function run() {
  ...
  if (errors.length > 0) {
    ...
    console.error('\nWidget Composite Validation Failed:\n')
    errors.forEach(e => console.error(`  x ${e}`))
    console.error('\nSee: ./widget-composite.spec.md for requirements\n')
    process.exit(2)
  }
