 ...
 * Widget Validator
 *
 * Validates files in creativeshire/components/content/widgets/
 *
 * Spec: ./widget.spec.md
 *
 * Exit codes:
...
const VALIDATOR_NAME = 'widget'

// Path pattern for this domain
// Use forward slashes - paths are normalized before matching
const DOMAIN_PATH = /creativeshire\/(?:components\/)?content\/widgets\//
 ...
 * From spec: "Default export from index.tsx"
 */
const checkDefaultExport: ValidationCheck = (content, filePath) => {
  if (filePath.endsWith('index.tsx') && !content.includes('export default')) {
    return 'Widget component must have a default export'
  }
  return null
}
 ...
 * From spec: "CSS variables have fallbacks: var(--name, fallback)"
 *
 * Valid:   var(--opacity, 1)
 * Invalid: var(--opacity)
 */
...
/**
 * Check: No direct will-change
 * From spec: "No direct will-change - use [data-behaviour] selectors only"
 */
const checkNoDirectWillChange: ValidationCheck = (content, filePath) => {
  if (!filePath.endsWith('.css') && !filePath.endsWith('.tsx')) return null

  // Allow will-change only in [data-behaviour] selectors
  const lines = content.split('\n')
  let inDataBehaviourSelector = false

  for (let i = 0; i < lines.length; i++) {
    const line = lines[i]

    // Track if we're inside a [data-behaviour] selector
    if (/\[data-behaviour/.test(line)) {
      inDataBehaviourSelector = true
    }
    if (inDataBehaviourSelector && line.includes('}')) {
      inDataBehaviourSelector = false
    }

    // Check for will-change outside [data-behaviour] selectors
    if (/will-change\s*:/.test(line) && !inDataBehaviourSelector) {
      return `will-change at line ${i + 1} must be inside [data-behaviour] selector`
    }
  }
...
function run() {
  ...
  if (errors.length > 0) {
    ...
    console.error('\nSee: ./widget.spec.md for requirements\n')
    process.exit(2)
  }
