 ...
 * Chrome Validator
 *
 * Validates files in creativeshire/components/content/chrome/
 *
 * Spec: ./chrome.spec.md
 *
 * Exit codes:
...
const VALIDATOR_NAME = 'chrome'

// Path pattern for this domain
// Use forward slashes - paths are normalized before matching
const DOMAIN_PATH = /creativeshire\/(?:components\/)?content\/chrome\//
 ...
 * From spec: "Default export from component files"
 */
const checkDefaultExport: ValidationCheck = (content, filePath) => {
  // Only check .tsx files that are components (not types.ts, index.ts)
  if (filePath.endsWith('.tsx') && !content.includes('export default')) {
    return 'Chrome component must have a default export'
  }
  return null
}
 ...
 */
const checkNoPositionFixed: ValidationCheck = (content, filePath) => {
  // Allow position: fixed in overlay CSS (cursor needs it for portal positioning)
  // but not in inline styles or component code
  if (filePath.endsWith('.tsx')) {
    if (/style\s*=\s*\{[^}]*position:\s*['"]?(?:fixed|sticky)/.test(content)) {
      return 'No inline position: fixed/sticky - handled by BehaviourWrapper'
    }
  }
  return null
}
 ...
 *
 * Valid:   var(--header-y, 0)
 * Invalid: var(--header-y)
 */
...
function run() {
  ...
  if (errors.length > 0) {
    ...
    console.error('\nSee: ./chrome.spec.md for requirements\n')
    process.exit(2)
  }
