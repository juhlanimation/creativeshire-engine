#!/usr/bin/env npx tsx
/**
 * Project Assistant Composite Validator
 *
 * Coordinator agent for task file management.
 * Restricts writes to task and documentation files only.
 *
 * Exit codes:
...
const AGENT_NAME = 'project-assistant'
 ...
 * Coordinator agents don't chain to domain validators.
 */
const domainValidators: string[] = []

/**
 * Path rules for this agent.
 * Allowlist mode: only specified paths can be written.
 * Task files and documentation only - no code files.
 */
const pathRules: {
  ...
  patterns: [
    /\.claude\/tasks\//,           // Task management files
    /\.claude\/development\//,     // Development documentation
    /\.claude\/architecture\/.*\.md$/,  // Architecture documentation
  ],

  message: 'Project Assistant can only edit task files (.claude/tasks/), development docs, and architecture documentation.',
}
