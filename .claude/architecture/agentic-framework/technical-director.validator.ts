#!/usr/bin/env npx tsx
/**
 * Technical Director Composite Validator
 *
 * Enforces path rules for the coordination agent.
 * Lives next to agent contract: ./technical-director.agent.md
 *
 * This file is the wiring layer (HOW) that connects an agent to:
 * - Path rules (WHERE the agent can write)
 *
 * The Technical Director is a coordinator - it delegates implementation
 * to domain specialists and must NOT write to creativeshire/ directly.
 *
 * Exit codes:
...
const AGENT_NAME = 'technical-director'
 ...
 * Technical Director is a coordinator - no domain validators needed.
 */
const domainValidators: string[] = []

/**
 * Path rules for this agent.
 * Technical Director uses blocklist mode:
 * - Block all creativeshire/ code (delegate to specialists)
 * - Allow task management and architecture docs
 */
const pathRules: {
  ...
  patterns: [
    // Block all creativeshire code
    /creativeshire\//,
    // Block site instance data
    /site\//,
    // Block app routing
    /app\//,
    // Block validator code (only path rules enforced)
    /\.validator\.ts$/,
  ],

  message: 'Technical Director cannot write implementation code. Delegate to the appropriate specialist agent.',
}
