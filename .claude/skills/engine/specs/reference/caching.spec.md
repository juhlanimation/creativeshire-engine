...
| `engine/content/` | **No** | Pure render, no data fetching |
| `engine/experience/` | **No** | Client-side animation |
| `engine/renderer/` | **No** | Receives data as props |
...
site/pages/        →  "use cache"  →  Defines page schema
       ↓
engine/renderer/  →  (no cache)  →  Receives props, renders
       ↓
engine/content/   →  (no cache)  →  Pure components
       ↓
engine/experience/  →  (no cache)  →  Client-side animation
```
