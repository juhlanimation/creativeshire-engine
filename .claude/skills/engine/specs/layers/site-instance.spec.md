...
The Site Instance Layer contains this site's configuration and content. Sites extend presets using spread syntax, overriding specific values while inheriting defaults. The engine lives in `engine/`. Instance data lives in `site/`.

This separation enables:
- Extracting `engine/` to an npm package
...
import { RegionSchema } from '@/engine/schema'
...
- Import from `engine/schema`
- Import from `engine/presets`
- Import from `engine/content` (composites only)
...
- Import from `engine/experience` directly
