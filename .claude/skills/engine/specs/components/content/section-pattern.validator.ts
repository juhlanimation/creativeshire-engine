#!/usr/bin/env npx tsx
/**
 * Section Pattern Validator
 *
 * Validates files in engine/components/content/sections/patterns/
 *
 * Spec: ./section-pattern.spec.md
 *
 * Exit codes:
...
const VALIDATOR_NAME = 'section-pattern'

// Path pattern for this domain
// Use forward slashes - paths are normalized before matching
const DOMAIN_PATH = /engine\/(?:components\/)?content\/sections\/composites\//
...
const checkNoReactImports: ValidationCheck = (content, filePath) => {
  ...
  if (/import\s+(?:React|\{[^}]*\})\s+from\s+['"]react['"]/.test(content)) {
    return 'No React imports - patterns are pure data functions'
  }
  return null
}
...
const checkNoJsx: ValidationCheck = (content, filePath) => {
  ...
  if (/<[A-Z][a-zA-Z]*|<[a-z]+\s/.test(content)) {
    return 'No JSX syntax - patterns return schema objects, not React elements'
  }
  return null
}
...
function run() {
  ...
  if (errors.length > 0) {
    ...
    console.error('\nSection Pattern Validation Failed:\n')
    errors.forEach(e => console.error(`  x ${e}`))
    console.error('\nSee: ./section-pattern.spec.md for requirements\n')
    process.exit(2)
  }
